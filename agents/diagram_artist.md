...
| **Excalidraw: box text invisible in create_view** | `label` property on shapes does NOT render in the MCP viewer | Use separate standalone `text` elements positioned at each box center: `x = cx - (chars × fontSize × 0.6) / 2`, `y = cy - fontSize * 0.7` per line |
| **Excalidraw: text overflows box edge** | monospace chars are ~0.6× fontSize wide, not 0.5× | Use `0.6` as the width multiplier; for 300px wide box at fontSize 14, max ~35 chars per line |
| **Excalidraw: text is a dot when opening .excalidraw in browser** | `width`/`height` missing on `text` elements — canvas renderer collapses to 0×0; PNG exporter re-measures so export looks fine | Every `text` element MUST have `width = ceil(longest_line_chars × 0.60 × fontSize)` and `height = ceil(line_count × fontSize × 1.25)` |
| **Excalidraw: elements invisible / rendering issues** | Missing required fields: `angle`, `seed`, `version`, `versionNonce`, `isDeleted`, `groupIds`, `frameId`, `boundElements` | All elements need these fields; text also needs `textAlign`, `verticalAlign`, `containerId`, `originalText`, `autoResize`, `lineHeight` |
| **Excalidraw: wrong initial viewport in browser** | `scrollX`/`scrollY` formula is `screen = zoom × (scene + scroll)` | `scrollX = -(x_min - padding)`, `scrollY = -(y_min - padding)` at zoom=1.0; verified: scene x=180 with scrollX=-140 → screen x=40 ✓ |
